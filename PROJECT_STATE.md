...
基于 7 个月电商行为日志的用户价值与购买潜力分层：**未购用户 → 首购潜力识别；已购用户 → 价值分层与"跨月完全沉默"（观察月无任何行为）流失判定**；滚动时间外验证 + 两套逻辑回归基准证明标签稳健、概率分排序优于规则；**队列迁移分析回答"10 月标签在后续月份是保持还是转化"**。
...
| `tracked_users_list_Nov.csv` | 候选触达名单（11 月高潜力首购 17,612 人，10 列，入库） |
| `user_segments_all_Nov.csv` | 全量分层名单（11 月活跃用户 18.5 万 × 5 类标签，体积大不入库，Notebook 运行后生成） |
| `tests/` | 单元测试（12 个测试类 / 25 个用例）：`python -m unittest discover -s tests` |
...
- **名单（规则圈池 + LR 池内排序，历史窗口训练）**：候选池 = 规则认可人群（`POOL_SEGMENTS`，默认除"普通浏览"外全部，未购池=高潜力首购）；LR 概率分只在池内排名，`TopK_Flag` = 池内前 `POOL_TOP_RATIO`（默认 50%）——**k 是比例参数（ceil(池内人数×比例)），不再是目标群体总人数**。**LR 时间窗口（避免偷看答案）**：`score_nonbuyers_history` / `score_buyers_history` 用打分月之前的历史（特征月→次月标签）训练、当月打分（3 月窗口）；**10 月无法给出概率分**（最早建模月无更早历史，10 月不需要选人），打分最早从 11 月开始。无泄漏实验（训练10/11月→预测12月未购1月购买）显示 LR 7.40% vs 规则 6.80%（+0.6pp，远小于泄漏版 +3pp），支撑"规则解释层 + LR 池内排序层"方案（见数据分析报告 8.1/8.2）
...
- **滚动时间外验证（正式验证）**：实验一（未购人群）2019-10→11 组：高潜力首购 5,408 vs 普通浏览 12.85 万，11 月购买率 **13.30% vs 5.41%**（购买率差 +7.88pp，p=2.2e-131）；6/6 个月对显著，购买率差 **+4.5~+9.3pp**，跨月稳定
- **实验二**（沉默=流失）：5/5 个月组极显著，购买率差 **−19.9~−31.8pp**（p<1e-68），方向一致——"跨月完全沉默"是可靠流失信号
- **LR 基准**：未购人群 AUC 0.528→0.673、Top-k 首购率 13.30%→16.31%；滚动口径未购人群 AUC 0.673~0.722、已购人群 0.606~0.646，均稳定优于规则
- **名单**：候选触达名单 = 11 月高潜力首购 17,612 人（TopK_Flag=1 池内前 50% 8,806 人优先触达）；全量名单 = 11 月活跃用户 18.5 万 × 5 类标签
- **队列迁移（10 月基期，关键数字）**：
