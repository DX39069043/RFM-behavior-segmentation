...
基于 7 个月电商行为日志的用户价值与购买潜力分层：**未购用户 → 首购潜力识别；已购用户 → 价值分层与"跨月完全沉默"流失判定**；滚动时间外验证 + 两臂逻辑回归基准证明标签稳健、概率分排序优于规则；**队列迁移分析回答"10 月标签在后续月份是保持还是转化"**。
...
| `tests/` | 单元测试（12 个测试类 / 18 个用例）：`python -m unittest discover -s tests` |
...
- **队列迁移（`cohort_migration`）**：固定基期（BASE_MONTH='2019-10'）分层 → 逐月用**冻结基期阈值**重算同一批用户的标签（`segment_users(thresholds=...)`），返回"基期标签 × 月份 × 冻结标签占比"（'无任何活动' = 当月完全无行为）；只做标签保持/转化，不含状态迁移表
...
- **队列迁移（10 月基期，关键数字）**：
  - 已购三标签月度保持率低：次月 12%~17%，6 个月后降至 2%~6%（标签是"月度快照"，需按月刷新）
  - "无任何活动"（完全无行为）是主去向：所有标签占比 6 个月后升至 69%~78%
  - 高价值深度互动粘性最强：无任何活动占比最低（11 月 24%）、次月购买占比最高（45.9% = 常规 21.6% + 直购 7.6% + 深度互动 16.7%）
