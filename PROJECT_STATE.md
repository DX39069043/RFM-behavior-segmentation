...
- **名单（规则圈池 + LR 池内排序）**：候选池 = 规则认可人群（`POOL_SEGMENTS`，默认除"普通浏览"外全部，未购池=高潜力首购）；LR 概率分只在池内排名，`TopK_Flag` = 池内前 `POOL_TOP_RATIO`（默认 50%）——**k 是比例参数（ceil(池内人数×比例)），不再是目标群体总人数**；无泄漏实验（训练10/11月→预测12月未购1月购买）显示 LR 7.40% vs 规则 6.80%（+0.6pp，远小于泄漏版 +3pp），支撑"规则解释层 + LR 池内排序层"方案（见数据分析报告 8.1/8.2）
