...
display(HTML('<p style="color:#777">小提醒：这里说的"未购用户"，指 <b>10 月这个月里没有买过东西</b>的人'
             ...
             '（老客回流），不能直接当成"新客名单"。要不要严格区分"从没买过的人"，取决于运营是想拉新还是唤醒。</p>'))
...
**Cell 27（markdown）** —— 补充实验（8.1）：无泄漏 LR vs 人为规则（12 月 → 1 月）：

> ### 补充实验（8.1）：无泄漏逻辑回归 vs 人为规则（12 月 → 1 月）
>
> 上面的实验一 / 二 / 三用的 LR 都是"全量拟合"（用 11 月结果训练、再评估 11 月），模型等于"看过答案"。为了检验"逻辑回归是不是真的比人为规则好很多"，这里做一次**严格时间隔离**的对比实验——LR 训练时完全看不到被预测月份的结果，避免"提前得知答案"造成虚高：
>
> - **训练**：10 月未购用户（10 月行为特征）→ 11 月是否购买；11 月未购用户（11 月特征）→ 12 月是否购买（两批共 29.6 万人，平均购买率 5.72%）；
> - **预测**：对 12 月未购用户（20.4 万人）用 12 月行为特征打分，取概率最高的前 k 人（k = 12 月规则圈出的高潜力首购人数 21,243，两边同预算对比）；
> - **验证**：看这批人 1 月是否购买（训练时完全不可见）。
>
> | 方法 | 人数 | 1 月购买率 |
> |---|---:|---:|
> | 基线（全体 12 月未购用户） | 204,007 | 2.76% |
> | 人为规则（12 月高潜力首购） | 21,243 | **6.80%**（基线的 2.5 倍） |
> | LR Top-k（无泄漏训练） | 21,243 | **7.40%** |
> | └ 重合（LR 保留的规则高分用户） | 11,252（53%） | — |
> | └ 规则独有（LR 换掉的） | 9,991 | 5.21% |
> | └ LR 独有（规则漏掉的） | 9,991 | 6.51% |
>
> **结论**：在彻底排除"提前得知结果"之后，LR 依然比规则好（7.40% vs 6.80%，高 0.6 个百分点），但优势远小于之前演示口径的 +3 个百分点——那多出来的部分主要来自"全量拟合对结果的偷看"。同时，规则圈出的人购买率是基线的 2.5 倍，被 LR 换掉的人仍有基线的 1.9 倍——**人为规则本身就是有效的识别工具，而且完全讲得清**（加购未买 + 高探索 → 购物车挽回话术）。这支撑了本项目的双层设计：**规则负责可解释的分层与话术，LR 概率分负责在规则认可的候选池内做精细排序**，两者不是替代关系。

**这个补充实验在回答什么**：前面的实验一二三里 LR 是"看过答案"的全量拟合（评估口径 OOF 已校正，但训练时点与预测时点重叠），本实验把训练窗口严格提前（10/11 月）、预测 12 月用户 1 月购买，得到"无泄漏下的真实差距"——结论：LR 只比规则好 0.6pp，规则本身（基线 2.5 倍命中）非常有效且讲得清，支撑"规则解释层 + LR 池内排序"的名单方案（见《数据分析报告》8.1 / 8.2）。

---
**Cell 28（markdown）**：`## 汇总报告：各人群 11 月转化率对比（面板口径）`（小节标题）。
